	...
	public void decompress(DataInputStream in, DataOutputStream out) throws IOException {
		...
		try {
			...
			while (true) {
                ...
                // TODO This is where our previous hire left us in a hurry...

                /*
                 * So now we have the next 12bit word in 'currentWord' and the
                 * previous 12bit word in 'previousWord'.
                 * 
                 * 1) If we have an entry in our dictionary for 'currentWord',
                 * output the entry. And add a new entry (if we still have room)
                 * to the dictionary, just as the compressor would.
                 * 
                 * 2) If we don't have 'currentWord' in the dictionary, how could
                 * that happen? Well, that means that the compressor will add it
                 * this round, but we don't know what that is! But we do know
                 * that there was some overlap in the string being compressed,
                 * otherwise it wouldn't have triggered the dictionary so soon.
                 * Specifically, that overlap is that the string we're looking
                 * for has the same last character as it's first character. So,
                 * add that to the dictionary and output it!
                 */
